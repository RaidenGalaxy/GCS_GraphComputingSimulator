...
bool Cache::send(Request req) {
  debug("level %d req.addr %llx req.type %d, index %d, tag %lld",
...
void Cache::evictline(addr_type addr, bool dirty) {
...
std::pair<long, bool> Cache::invalidate(addr_type addr) {
...
void Cache::evict(std::list<Line>* lines,
  ...
  debug("level %d miss evict victim %llx", int(level), victim->addr);
  cache_eviction++;

  addr_type addr = victim->addr;
  ...
  if (!is_last_level) {
    ...
    if (dirty) {
      ...
      debug("inject one write request to memory system "
          "addr %llx, invalidate time %ld, issue time %ld",
...
bool Cache::is_hit(std::list<Line>& lines, addr_type addr,
...
bool Cache::need_eviction(const std::list<Line>& lines, addr_type addr) {
...
void CacheSystem::tick() {
  ...
  while (it != wait_list.end() && clk >= it->first) {
    if (!send_memory(it->second)) {
      ...
      debug("complete req: addr %llx", (it->second).addr);
  ...
  while (it != hit_list.end()) {
    if (clk >= it->first) {
      ...
      debug("finish hit: addr %llx", (it->second).addr);
