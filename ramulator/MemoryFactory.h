...
public:
    ...
    static Memory<T> *populate_memory(const Config& configs, T *spec, int channels, int ranks, int id) {
        ...
        for (int c = 0; c < channels; c++){
            ...
            channel->id = c;
            channel->regStats("");
            ctrls.push_back(new Controller<T>(configs, channel, id));
        }
    ...
    static MemoryBase *create(const Config& configs, int cacheline, int id)
    {
        ...
        return (MemoryBase *)populate_memory(configs, spec, channels, ranks, id);
    }
};
