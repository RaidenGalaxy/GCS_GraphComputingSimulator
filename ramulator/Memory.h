...
public:
    ...
    virtual void tick() = 0;
    virtual bool try_send_read(const Request& req) const = 0;
    virtual bool try_send_write(const Request& req) const = 0;
    virtual void init(Request& req) const = 0;
    virtual bool send(Request& req) = 0;
    virtual int pending_requests() = 0;
    virtual void finish(void) = 0;
    virtual addr_type page_allocator(addr_type addr, int coreid) = 0;
...
public:
    ...
    addr_type free_physical_pages_remaining;
    map<pair<int, addr_type>, addr_type> page_translation;
    ...
    bool try_send_read(const Request& req) const {
        return ctrls[req.addr_vec[0]]->try_enqueue_read();
    }

    bool try_send_write(const Request& req) const {
        return ctrls[req.addr_vec[0]]->try_enqueue_write();
    }
    ...
    void apply_mapping(addr_type addr, std::vector<int>& addr_vec){
        int *sz = spec->org_entry.count;
        ...
        for (unsigned int lvl = 0; lvl < int(T::Level::MAX); lvl++)
        {
            ...
            for (unsigned int bitindex = 0 ; bitindex < lvl_bits ; bitindex++){
                ...
                {
                    bitvalue = bitvalue ^ get_bit_at(addr, *it);
                }
    ...
    addr_type page_allocator(addr_type addr, int coreid) {
        addr_type virtual_page_number = addr >> 12;
        ...
        switch(int(translation)) {
            ...
            default:
                assert(false); return static_cast<addr_type>(-1);
        }
...
private:
    ...
    int slice_lower_bits(addr_type& addr, int bits) const
    {
        int lbits = addr & ((1<<bits) - 1);
        addr >>= bits;
        return lbits;
    }
    bool get_bit_at(addr_type addr, int bit)
    {
        return (((addr >> bit) & 1) == 1);
    }
    void clear_lower_bits(addr_type& addr, int bits) const
    {
        addr >>= bits;
    }
    long lrand(void) {
