...
public:
    ...
    /* Constructor */
    Controller(const Config& configs, DRAM<T>* channel, long id) :
		id(id),
        channel(channel),
        ...
        cmd_trace_files(channel->children.size())
    {
        ...
        if (record_cmd_trace){
            ...
            string prefix = cmd_trace_prefix + to_string(id) + "-chan-" + to_string(channel->id) + "-rank-";
    ...
    const Queue& get_queue(Request::Type type) const 
    {
        switch (int(type)) {
            case int(Request::Type::READ) : return readq;
                case int(Request::Type::WRITE) : return writeq;
    ...
    bool try_enqueue_read() const {
        return readq.max != readq.size();
    }

    bool try_enqueue_write() const {
        return writeq.max != writeq.size();
    }
    ...
    bool enqueue(Request& req)
    {
        ...
        if (req.type == Request::Type::READ) {
            for (const Request& wreq : writeq.q) {
                if (req.addr == wreq.addr) {
                    req.depart = clk + 1;
    ...
    void tick()
    {
        ...
        if (!write_mode) {
            // yes -- write queue is almost full or read queue is end
            if (writeq.size() > int(wr_high_watermark * writeq.max) || readq.size() == 0)
                write_mode = true;
        }
        else {
            // no -- write queue is almost end and read queue is not end
