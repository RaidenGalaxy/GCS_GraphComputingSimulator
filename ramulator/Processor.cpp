...
void Window::insert(bool ready, addr_type addr)
{
    assert(load <= depth);
...
void Window::set_ready(addr_type addr, int mask)
{
    if (load == 0) return;
...
bool Trace::get_unfiltered_request(long& bubble_cnt, addr_type& req_addr, Request::Type& req_type)
{
    string line;
    getline(file, line);
    if (file.eof()) {
    ...
    req_addr = std::stoull(line.substr(pos), &end, 0);
...
bool Trace::get_filtered_request(long& bubble_cnt, addr_type& req_addr, Request::Type& req_type)
{
    ...
    pos = line.find_first_not_of(' ', pos+1);
    req_addr = stoull(line.substr(pos), &end, 0);
    req_type = Request::Type::READ;

    pos = line.find_first_not_of(' ', pos+end);
    if (pos != string::npos){
        has_write = true;
        write_addr = stoull(line.substr(pos), NULL, 0);
    }
    return true;
}
...
bool Trace::get_dramtrace_request(long& req_addr, Request::Type& req_type)
{
    ...
    size_t pos;
    req_addr = std::stoull(line, &pos, 16);
