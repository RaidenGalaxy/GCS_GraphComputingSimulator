...
public:
  ...
  struct Line {
    ...
    bool dirty;
    Line(addr_type addr, idx_type tag):
        addr(addr), tag(tag), lock(true), dirty(false) {}
    Line(addr_type addr, idx_type tag, bool lock, bool dirty):
...
protected:
  ...
  std::vector<std::pair<addr_type, std::list<Line>::iterator>> mshr_entries;
  ...
  int get_index(addr_type addr) {
    return (addr >> index_offset) & index_mask;
  };

  idx_type get_tag(addr_type addr) {
  ...
  void evictline(addr_type addr, bool dirty);
  ...
  std::pair<long, bool> invalidate(addr_type addr);
  ...
  bool check_unlock(addr_type addr) {
  ...
  std::vector<std::pair<addr_type, std::list<Line>::iterator>>::iterator
  hit_mshr(addr_type addr) {
    auto mshr_it =
  ...
  std::list<Line>& get_lines(addr_type addr) {
